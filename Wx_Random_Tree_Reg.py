...
target_forecast = 3
sounding_level = 20000 #in pascals
...
merged_data = merged_data.merge(sounding_entry(Sounding_filepath, sounding_data_type, sounding_level), left_index = True, right_index=True)
...
#tree_reg.fit(model_data[target_forecast:], model_labels[target_forecast:])

scores = cross_val_score(forest_reg, model_data[target_forecast:], model_labels[target_forecast:], 
                         scoring = "neg_mean_squared_error", cv=10)
#model_predictions = tree_reg.predict(model_data)
#tree_mse = mean_squared_error(model_labels[target_forecast:], model_predictions[target_forecast:])
